...
class EvalProblem:
    ...
    # Definitions the STATEMENT itself depends on — e.g. the IMO 2026 problems
    # define `Board`, `Move`, `Reachable`, `Mval` before their theorem, and the
    # theorem is meaningless without them.
    #
    # Not the same thing as reference_preamble, which holds the *solution's*
    # helper lemmas and is deliberately withheld from the prover. This one is
    # part of the problem, so it IS shown (see full_statement) — the model
    # cannot reason about `Move` without knowing what a Move is.
    #
    # Stored separately because it must be committed to the Lean environment
    # as its own command before the theorem is elaborated. A proofState does
    # not carry declarations made in the same command as the theorem, so
    # otherwise EVERY tactic mentioning one fails with "Unknown identifier" —
    # including `exact h`, since elaborating the existing context needs the
    # names. That made all 15 such problems unprovable by construction rather
    # than merely hard; the 2026 run's 0/3 measured this bug, not the model.
    statement_preamble: str = ""
    # Where the statement came from, e.g. "Mathlib Archive/Imo/Imo1959Q1.lean".
    source: str = ""

    @property
    def full_statement(self) -> str:
        """Statement as a human reads it: definitions followed by the theorem.

        This is what the director is shown. Lean gets the two parts
        separately (see statement_preamble)."""
        if not self.statement_preamble:
            return self.statement
        return f"{self.statement_preamble}\n\n{self.statement}"
...
PROBLEMS: list[EvalProblem] = [
    ...
    # -------------------------------------------------------------------------
    # IMO 2026 — all six problems of the 2026 olympiad, in the Lean
    # formalisation published by Axiom Math (github.com/AxiomMath/IMO2026).
    # Definitions travel inside the statement because the theorems say nothing
    # without them. Every statement was checked to elaborate to a real open
    # goal here, and every upstream proof was replayed here with no sorries
    # and standard axioms only — so a failure on these is the prover's, not a
    # mis-stated problem's.
    # -------------------------------------------------------------------------

    # IMO 2026 P1(a), termination. A board holds 2026 integers, each > 1; a move replaces two
    # entries m, n by gcd(m,n) and lcm(m,n)/gcd(m,n). No infinite play exists from an initial
    # board.
    EvalProblem(
        name="imo2026_q1a_termination",
        statement_preamble="set_option backward.isDefEq.respectTransparency false\n\n/-- A *board* is a finite multiset of natural numbers.  The full board discipline\n(entries `≥ 1`, cardinality `2026`) is captured by the predicate `IsInitial`. -/\nabbrev Board := Multiset ℕ\n\n/-- An *initial board*: exactly `2026` entries, each strictly greater than `1`. -/\ndef IsInitial (B : Board) : Prop :=\n  Multiset.card B = 2026 ∧ ∀ a ∈ B, 1 < a\n\n/-- A single *move*: pick two entries `m, n` (from two distinct positions,\nmodelled as two separate elements of the multiset) both `> 1`, remove them and\ninsert `gcd(m, n)` and `lcm(m, n) / gcd(m, n)`.  Using `m ::ₘ n ::ₘ s` for the\nsource board automatically encodes that the two chosen positions are distinct\n(they are two separate multiset elements, whose *values* may coincide). -/\ndef Move (B B' : Board) : Prop :=\n  ∃ (m n : ℕ) (s : Board), 1 < m ∧ 1 < n ∧\n    B = m ::ₘ n ::ₘ s ∧\n    B' = Nat.gcd m n ::ₘ (Nat.lcm m n / Nat.gcd m n) ::ₘ s\n\n/-- A board is *terminal* when at most one entry is `> 1`, so no move is possible. -/\ndef IsTerminal (B : Board) : Prop :=\n  Multiset.card (B.filter (fun a => 1 < a)) ≤ 1\n\n/-- A board has a *unique large entry* when exactly one entry is `> 1`. -/\ndef HasUniqueLarge (B : Board) : Prop :=\n  Multiset.card (B.filter (fun a => 1 < a)) = 1\n\n/-- `Reachable B B'` : `B'` can be obtained from `B` by a finite sequence of moves\n(the reflexive–transitive closure of `Move`).  A finite play from `B` to a\nterminal board `B'` is precisely a witness of `Reachable B B'` with `IsTerminal B'`. -/\ndef Reachable (B B' : Board) : Prop := Relation.ReflTransGen Move B B'\n\n/-- The exponent `g_p` for a prime `p` and board `B`: the `gcd` of the `p`-adic\nvaluations of the entries of `B`.  Since `gcd(a, 0) = a`, valuations equal to `0`\n(entries not divisible by `p`) do not affect this gcd, so `gExp p B` is the gcd of\nthe *positive* `p`-adic valuations occurring in `B`. -/\nnoncomputable def gExp (p : ℕ) (B : Board) : ℕ :=\n  (B.map (fun a => padicValNat p a)).gcd\n\n/-- The claimed invariant terminal value\n`M = ∏_{p ∣ ∏ B} p ^ gExp p B`, the product over all primes dividing some entry\nof `B` of `p` raised to the gcd of the `p`-adic valuations. -/\nnoncomputable def Mval (B : Board) : ℕ :=\n  ∏ p ∈ B.prod.primeFactors, p ^ gExp p B",
        statement="theorem statement_a_termination (B₀ : Board) (hB₀ : IsInitial B₀) :\n    ¬ ∃ f : ℕ → Board, f 0 = B₀ ∧ ∀ k, Move (f k) (f (k + 1)) := by",
        difficulty="imo",
        description="IMO 2026 P1(a), termination. A board holds 2026 integers, each > 1; a move replaces two entries m, n by gcd(m,n) and lcm(m,n)/gcd(m,n). No infinite play exists from an initial board.",
        tags=["imo", "imo2026", "q1", "number-theory", "gcd", "termination"],
        source="benchmarks/imo2026/IMO2026/Q1/problem.lean (statement_a_termination)",
    ),
    # IMO 2026 P1(a), part 2. Every terminal board reachable from an initial board has exactly
    # one entry greater than 1.
    EvalProblem(
        name="imo2026_q1a_unique_large",
        statement_preamble="set_option backward.isDefEq.respectTransparency false\n\n/-- A *board* is a finite multiset of natural numbers.  The full board discipline\n(entries `≥ 1`, cardinality `2026`) is captured by the predicate `IsInitial`. -/\nabbrev Board := Multiset ℕ\n\n/-- An *initial board*: exactly `2026` entries, each strictly greater than `1`. -/\ndef IsInitial (B : Board) : Prop :=\n  Multiset.card B = 2026 ∧ ∀ a ∈ B, 1 < a\n\n/-- A single *move*: pick two entries `m, n` (from two distinct positions,\nmodelled as two separate elements of the multiset) both `> 1`, remove them and\ninsert `gcd(m, n)` and `lcm(m, n) / gcd(m, n)`.  Using `m ::ₘ n ::ₘ s` for the\nsource board automatically encodes that the two chosen positions are distinct\n(they are two separate multiset elements, whose *values* may coincide). -/\ndef Move (B B' : Board) : Prop :=\n  ∃ (m n : ℕ) (s : Board), 1 < m ∧ 1 < n ∧\n    B = m ::ₘ n ::ₘ s ∧\n    B' = Nat.gcd m n ::ₘ (Nat.lcm m n / Nat.gcd m n) ::ₘ s\n\n/-- A board is *terminal* when at most one entry is `> 1`, so no move is possible. -/\ndef IsTerminal (B : Board) : Prop :=\n  Multiset.card (B.filter (fun a => 1 < a)) ≤ 1\n\n/-- A board has a *unique large entry* when exactly one entry is `> 1`. -/\ndef HasUniqueLarge (B : Board) : Prop :=\n  Multiset.card (B.filter (fun a => 1 < a)) = 1\n\n/-- `Reachable B B'` : `B'` can be obtained from `B` by a finite sequence of moves\n(the reflexive–transitive closure of `Move`).  A finite play from `B` to a\nterminal board `B'` is precisely a witness of `Reachable B B'` with `IsTerminal B'`. -/\ndef Reachable (B B' : Board) : Prop := Relation.ReflTransGen Move B B'\n\n/-- The exponent `g_p` for a prime `p` and board `B`: the `gcd` of the `p`-adic\nvaluations of the entries of `B`.  Since `gcd(a, 0) = a`, valuations equal to `0`\n(entries not divisible by `p`) do not affect this gcd, so `gExp p B` is the gcd of\nthe *positive* `p`-adic valuations occurring in `B`. -/\nnoncomputable def gExp (p : ℕ) (B : Board) : ℕ :=\n  (B.map (fun a => padicValNat p a)).gcd\n\n/-- The claimed invariant terminal value\n`M = ∏_{p ∣ ∏ B} p ^ gExp p B`, the product over all primes dividing some entry\nof `B` of `p` raised to the gcd of the `p`-adic valuations. -/\nnoncomputable def Mval (B : Board) : ℕ :=\n  ∏ p ∈ B.prod.primeFactors, p ^ gExp p B",
        statement="theorem statement_a_unique_large (B₀ : Board) (hB₀ : IsInitial B₀)\n    (B' : Board) (hreach : Reachable B₀ B') (hterm : IsTerminal B') :\n    HasUniqueLarge B' := by",
        difficulty="imo",
        description="IMO 2026 P1(a), part 2. Every terminal board reachable from an initial board has exactly one entry greater than 1.",
        tags=["imo", "imo2026", "q1", "number-theory", "gcd"],
        source="benchmarks/imo2026/IMO2026/Q1/problem.lean (statement_a_unique_large)",
    ),
    # IMO 2026 P1(b). Any two terminal boards reachable from the same initial board carry the
    # same entries > 1 — the surviving value is an invariant of the starting board.
    EvalProblem(
        name="imo2026_q1b_invariance",
        statement_preamble="set_option backward.isDefEq.respectTransparency false\n\n/-- A *board* is a finite multiset of natural numbers.  The full board discipline\n(entries `≥ 1`, cardinality `2026`) is captured by the predicate `IsInitial`. -/\nabbrev Board := Multiset ℕ\n\n/-- An *initial board*: exactly `2026` entries, each strictly greater than `1`. -/\ndef IsInitial (B : Board) : Prop :=\n  Multiset.card B = 2026 ∧ ∀ a ∈ B, 1 < a\n\n/-- A single *move*: pick two entries `m, n` (from two distinct positions,\nmodelled as two separate elements of the multiset) both `> 1`, remove them and\ninsert `gcd(m, n)` and `lcm(m, n) / gcd(m, n)`.  Using `m ::ₘ n ::ₘ s` for the\nsource board automatically encodes that the two chosen positions are distinct\n(they are two separate multiset elements, whose *values* may coincide). -/\ndef Move (B B' : Board) : Prop :=\n  ∃ (m n : ℕ) (s : Board), 1 < m ∧ 1 < n ∧\n    B = m ::ₘ n ::ₘ s ∧\n    B' = Nat.gcd m n ::ₘ (Nat.lcm m n / Nat.gcd m n) ::ₘ s\n\n/-- A board is *terminal* when at most one entry is `> 1`, so no move is possible. -/\ndef IsTerminal (B : Board) : Prop :=\n  Multiset.card (B.filter (fun a => 1 < a)) ≤ 1\n\n/-- A board has a *unique large entry* when exactly one entry is `> 1`. -/\ndef HasUniqueLarge (B : Board) : Prop :=\n  Multiset.card (B.filter (fun a => 1 < a)) = 1\n\n/-- `Reachable B B'` : `B'` can be obtained from `B` by a finite sequence of moves\n(the reflexive–transitive closure of `Move`).  A finite play from `B` to a\nterminal board `B'` is precisely a witness of `Reachable B B'` with `IsTerminal B'`. -/\ndef Reachable (B B' : Board) : Prop := Relation.ReflTransGen Move B B'\n\n/-- The exponent `g_p` for a prime `p` and board `B`: the `gcd` of the `p`-adic\nvaluations of the entries of `B`.  Since `gcd(a, 0) = a`, valuations equal to `0`\n(entries not divisible by `p`) do not affect this gcd, so `gExp p B` is the gcd of\nthe *positive* `p`-adic valuations occurring in `B`. -/\nnoncomputable def gExp (p : ℕ) (B : Board) : ℕ :=\n  (B.map (fun a => padicValNat p a)).gcd\n\n/-- The claimed invariant terminal value\n`M = ∏_{p ∣ ∏ B} p ^ gExp p B`, the product over all primes dividing some entry\nof `B` of `p` raised to the gcd of the `p`-adic valuations. -/\nnoncomputable def Mval (B : Board) : ℕ :=\n  ∏ p ∈ B.prod.primeFactors, p ^ gExp p B",
        statement="theorem statement_b_invariance (B₀ : Board) (hB₀ : IsInitial B₀)\n    (B₁ B₂ : Board) (h₁ : Reachable B₀ B₁) (h₂ : Reachable B₀ B₂)\n    (t₁ : IsTerminal B₁) (t₂ : IsTerminal B₂) :\n    ∀ M, (1 < M ∧ M ∈ B₁) ↔ (1 < M ∧ M ∈ B₂) := by",
        difficulty="imo",
        description="IMO 2026 P1(b). Any two terminal boards reachable from the same initial board carry the same entries > 1 — the surviving value is an invariant of the starting board.",
        tags=["imo", "imo2026", "q1", "number-theory", "gcd", "invariant"],
        source="benchmarks/imo2026/IMO2026/Q1/problem.lean (statement_b_invariance)",
    ),
    # IMO 2026 P1, explicit value. The surviving entry M of any terminal board equals ∏_p p ^
    # (gcd of the p-adic valuations of the initial entries).
    EvalProblem(
        name="imo2026_q1_terminal_value",
        statement_preamble="set_option backward.isDefEq.respectTransparency false\n\n/-- A *board* is a finite multiset of natural numbers.  The full board discipline\n(entries `≥ 1`, cardinality `2026`) is captured by the predicate `IsInitial`. -/\nabbrev Board := Multiset ℕ\n\n/-- An *initial board*: exactly `2026` entries, each strictly greater than `1`. -/\ndef IsInitial (B : Board) : Prop :=\n  Multiset.card B = 2026 ∧ ∀ a ∈ B, 1 < a\n\n/-- A single *move*: pick two entries `m, n` (from two distinct positions,\nmodelled as two separate elements of the multiset) both `> 1`, remove them and\ninsert `gcd(m, n)` and `lcm(m, n) / gcd(m, n)`.  Using `m ::ₘ n ::ₘ s` for the\nsource board automatically encodes that the two chosen positions are distinct\n(they are two separate multiset elements, whose *values* may coincide). -/\ndef Move (B B' : Board) : Prop :=\n  ∃ (m n : ℕ) (s : Board), 1 < m ∧ 1 < n ∧\n    B = m ::ₘ n ::ₘ s ∧\n    B' = Nat.gcd m n ::ₘ (Nat.lcm m n / Nat.gcd m n) ::ₘ s\n\n/-- A board is *terminal* when at most one entry is `> 1`, so no move is possible. -/\ndef IsTerminal (B : Board) : Prop :=\n  Multiset.card (B.filter (fun a => 1 < a)) ≤ 1\n\n/-- A board has a *unique large entry* when exactly one entry is `> 1`. -/\ndef HasUniqueLarge (B : Board) : Prop :=\n  Multiset.card (B.filter (fun a => 1 < a)) = 1\n\n/-- `Reachable B B'` : `B'` can be obtained from `B` by a finite sequence of moves\n(the reflexive–transitive closure of `Move`).  A finite play from `B` to a\nterminal board `B'` is precisely a witness of `Reachable B B'` with `IsTerminal B'`. -/\ndef Reachable (B B' : Board) : Prop := Relation.ReflTransGen Move B B'\n\n/-- The exponent `g_p` for a prime `p` and board `B`: the `gcd` of the `p`-adic\nvaluations of the entries of `B`.  Since `gcd(a, 0) = a`, valuations equal to `0`\n(entries not divisible by `p`) do not affect this gcd, so `gExp p B` is the gcd of\nthe *positive* `p`-adic valuations occurring in `B`. -/\nnoncomputable def gExp (p : ℕ) (B : Board) : ℕ :=\n  (B.map (fun a => padicValNat p a)).gcd\n\n/-- The claimed invariant terminal value\n`M = ∏_{p ∣ ∏ B} p ^ gExp p B`, the product over all primes dividing some entry\nof `B` of `p` raised to the gcd of the `p`-adic valuations. -/\nnoncomputable def Mval (B : Board) : ℕ :=\n  ∏ p ∈ B.prod.primeFactors, p ^ gExp p B",
        statement="theorem terminal_value_eq_Mval (B₀ : Board) (hB₀ : IsInitial B₀)\n    (B' : Board) (hreach : Reachable B₀ B') (hterm : IsTerminal B')\n    (M : ℕ) (hM : 1 < M) (hMem : M ∈ B') :\n    M = Mval B₀ := by",
        difficulty="imo",
        description="IMO 2026 P1, explicit value. The surviving entry M of any terminal board equals ∏_p p ^ (gcd of the p-adic valuations of the initial entries).",
        tags=["imo", "imo2026", "q1", "number-theory", "gcd", "padic"],
        source="benchmarks/imo2026/IMO2026/Q1/problem.lean (terminal_value_eq_Mval)",
    ),
    # IMO 2026 P1, auxiliary. The invariant terminal value M is itself greater than 1.
    EvalProblem(
        name="imo2026_q1_mval_gt_one",
        statement_preamble="set_option backward.isDefEq.respectTransparency false\n\n/-- A *board* is a finite multiset of natural numbers.  The full board discipline\n(entries `≥ 1`, cardinality `2026`) is captured by the predicate `IsInitial`. -/\nabbrev Board := Multiset ℕ\n\n/-- An *initial board*: exactly `2026` entries, each strictly greater than `1`. -/\ndef IsInitial (B : Board) : Prop :=\n  Multiset.card B = 2026 ∧ ∀ a ∈ B, 1 < a\n\n/-- A single *move*: pick two entries `m, n` (from two distinct positions,\nmodelled as two separate elements of the multiset) both `> 1`, remove them and\ninsert `gcd(m, n)` and `lcm(m, n) / gcd(m, n)`.  Using `m ::ₘ n ::ₘ s` for the\nsource board automatically encodes that the two chosen positions are distinct\n(they are two separate multiset elements, whose *values* may coincide). -/\ndef Move (B B' : Board) : Prop :=\n  ∃ (m n : ℕ) (s : Board), 1 < m ∧ 1 < n ∧\n    B = m ::ₘ n ::ₘ s ∧\n    B' = Nat.gcd m n ::ₘ (Nat.lcm m n / Nat.gcd m n) ::ₘ s\n\n/-- A board is *terminal* when at most one entry is `> 1`, so no move is possible. -/\ndef IsTerminal (B : Board) : Prop :=\n  Multiset.card (B.filter (fun a => 1 < a)) ≤ 1\n\n/-- A board has a *unique large entry* when exactly one entry is `> 1`. -/\ndef HasUniqueLarge (B : Board) : Prop :=\n  Multiset.card (B.filter (fun a => 1 < a)) = 1\n\n/-- `Reachable B B'` : `B'` can be obtained from `B` by a finite sequence of moves\n(the reflexive–transitive closure of `Move`).  A finite play from `B` to a\nterminal board `B'` is precisely a witness of `Reachable B B'` with `IsTerminal B'`. -/\ndef Reachable (B B' : Board) : Prop := Relation.ReflTransGen Move B B'\n\n/-- The exponent `g_p` for a prime `p` and board `B`: the `gcd` of the `p`-adic\nvaluations of the entries of `B`.  Since `gcd(a, 0) = a`, valuations equal to `0`\n(entries not divisible by `p`) do not affect this gcd, so `gExp p B` is the gcd of\nthe *positive* `p`-adic valuations occurring in `B`. -/\nnoncomputable def gExp (p : ℕ) (B : Board) : ℕ :=\n  (B.map (fun a => padicValNat p a)).gcd\n\n/-- The claimed invariant terminal value\n`M = ∏_{p ∣ ∏ B} p ^ gExp p B`, the product over all primes dividing some entry\nof `B` of `p` raised to the gcd of the `p`-adic valuations. -/\nnoncomputable def Mval (B : Board) : ℕ :=\n  ∏ p ∈ B.prod.primeFactors, p ^ gExp p B",
        statement="theorem Mval_gt_one (B₀ : Board) (hB₀ : IsInitial B₀) : 1 < Mval B₀ := by",
        difficulty="imo",
        description="IMO 2026 P1, auxiliary. The invariant terminal value M is itself greater than 1.",
        tags=["imo", "imo2026", "q1", "number-theory", "gcd", "spec"],
        source="benchmarks/imo2026/IMO2026/Q1/problem.lean (Mval_gt_one)",
    ),
    # IMO 2026 P2 (geometry). K lies inside triangle BMC and L inside BNC, pinned by three
    # angle equalities; M, N are the midpoints of AB, AC. The circumcentre O of triangle AKL
    # satisfies OM = ON.
    EvalProblem(
        name="imo2026_q2",
        ...
        difficulty="imo",
        description="IMO 2026 P2 (geometry). K lies inside triangle BMC and L inside BNC, pinned by three angle equalities; M, N are the midpoints of AB, AC. The circumcentre O of triangle AKL satisfies OM = ON.",
        tags=["imo", "imo2026", "q2", "geometry", "euclidean", "circumcentre"],
        source="benchmarks/imo2026/IMO2026/Q2/problem.lean (main_theorem)",
    ),
    # IMO 2026 P3, specification lemma. Cutting the unit stick at a finite set of interior
    # points yields pieces whose lengths sum to 1.
    EvalProblem(
        name="imo2026_q3_spec_sum",
        ...
        statement="theorem pieceLengths_sum (S : Finset ℝ) (hS : ↑S ⊆ Set.Ioo (0 : ℝ) 1) :\n    (pieceLengths S).sum = 1 := by",
        difficulty="imo",
        description="IMO 2026 P3, specification lemma. Cutting the unit stick at a finite set of interior points yields pieces whose lengths sum to 1.",
        tags=["imo", "imo2026", "q3", "combinatorics", "spec-lemma", "real", "spec"],
        source="benchmarks/imo2026/IMO2026/Q3/problem.lean (pieceLengths_sum)",
    ),
    # IMO 2026 P3, specification lemma. Cutting at |S| points yields exactly |S| + 1 pieces.
    EvalProblem(
        name="imo2026_q3_spec_length",
        statement_preamble="set_option backward.isDefEq.respectTransparency false\n\nopen scoped BigOperators\n\n\n/-- The multiset of piece lengths obtained by cutting `[0,1]` at the points of a\nfinite set `S ⊆ (0,1)`.  We sort `S` ascending, prepend `0` and append `1`, and\ntake consecutive differences.  The result is a list of `|S| + 1` positive reals\nsumming to `1` (when `S ⊆ (0,1)`). -/\nnoncomputable def pieceLengths (S : Finset ℝ) : List ℝ :=\n  let l : List ℝ := (0 : ℝ) :: (S.sort (· ≤ ·)) ++ [1]\n  List.zipWith (fun a b => b - a) l l.tail\n\n/-- The sum of the entries of a list `L` at the (0-indexed) even positions, after\nsorting `L` in non-increasing order.  These are the entries in the `1`st, `3`rd,\n`5`th, … positions of the sorted (decreasing) list, i.e. the pieces claimed by\nthe first mover under the greedy claiming rule. -/\nnoncomputable def firstPlayerShare (L : List ℝ) : ℝ :=\n  let sorted := L.mergeSort (· ≥ ·)\n  ((sorted.zipIdx.filter (fun p => p.2 % 2 = 0)).map (fun p => p.1)).sum\n\n/-- `L(A,B)`: Liu Bang's total length, given Liu Bang's marks `A` and Xiang Yu's\nmarks `B`. -/\nnoncomputable def L (A B : Finset ℝ) : ℝ :=\n  firstPlayerShare (pieceLengths (A ∪ B))\n\n/-- The set of admissible markings for a player: a finite subset of `(0,1)` of\nsize at most `n`.  We encode it as a `Finset ℝ` subject to the side conditions. -/\ndef AdmissibleMark (n : ℕ) (X : Finset ℝ) : Prop :=\n  (↑X ⊆ Set.Ioo (0 : ℝ) 1) ∧ X.card ≤ n\n\n/-- The value Liu Bang can guarantee.\n\n`V n` is the supremum over Liu Bang's admissible markings `A` of the infimum,\nover Xiang Yu's admissible markings `B` disjoint from `A`, of `L A B`. -/\nnoncomputable def V (n : ℕ) : ℝ :=\n  ⨆ A : {A : Finset ℝ // AdmissibleMark n A},\n    ⨅ B : {B : Finset ℝ // AdmissibleMark n B ∧ Disjoint A.1 B}, L A.1 B.1\n\n/-- The claimed answer value `V(n) = 2^n / (2^(n+1) - 1)`. -/\nnoncomputable def answer (n : ℕ) : ℝ := (2 : ℝ) ^ n / ((2 : ℝ) ^ (n + 1) - 1)\n\n/-! ## Correctness statements for the definitions\n\nThese pin down that the encoded definitions behave as intended. -/\n\n\n\n\n/-! ## Main Statements -/",
        statement="theorem pieceLengths_length (S : Finset ℝ) :\n    (pieceLengths S).length = S.card + 1 := by",
        difficulty="imo",
        description="IMO 2026 P3, specification lemma. Cutting at |S| points yields exactly |S| + 1 pieces.",
        tags=["imo", "imo2026", "q3", "combinatorics", "spec-lemma", "list", "spec"],
        source="benchmarks/imo2026/IMO2026/Q3/problem.lean (pieceLengths_length)",
    ),
    # IMO 2026 P3, specification lemma. Liu Bang's share is a subset-sum of the piece lengths,
    # hence lies in [0, 1].
    EvalProblem(
        name="imo2026_q3_spec_share_bounds",
        statement_preamble="set_option backward.isDefEq.respectTransparency false\n\nopen scoped BigOperators\n\n\n/-- The multiset of piece lengths obtained by cutting `[0,1]` at the points of a\nfinite set `S ⊆ (0,1)`.  We sort `S` ascending, prepend `0` and append `1`, and\ntake consecutive differences.  The result is a list of `|S| + 1` positive reals\nsumming to `1` (when `S ⊆ (0,1)`). -/\nnoncomputable def pieceLengths (S : Finset ℝ) : List ℝ :=\n  let l : List ℝ := (0 : ℝ) :: (S.sort (· ≤ ·)) ++ [1]\n  List.zipWith (fun a b => b - a) l l.tail\n\n/-- The sum of the entries of a list `L` at the (0-indexed) even positions, after\nsorting `L` in non-increasing order.  These are the entries in the `1`st, `3`rd,\n`5`th, … positions of the sorted (decreasing) list, i.e. the pieces claimed by\nthe first mover under the greedy claiming rule. -/\nnoncomputable def firstPlayerShare (L : List ℝ) : ℝ :=\n  let sorted := L.mergeSort (· ≥ ·)\n  ((sorted.zipIdx.filter (fun p => p.2 % 2 = 0)).map (fun p => p.1)).sum\n\n/-- `L(A,B)`: Liu Bang's total length, given Liu Bang's marks `A` and Xiang Yu's\nmarks `B`. -/\nnoncomputable def L (A B : Finset ℝ) : ℝ :=\n  firstPlayerShare (pieceLengths (A ∪ B))\n\n/-- The set of admissible markings for a player: a finite subset of `(0,1)` of\nsize at most `n`.  We encode it as a `Finset ℝ` subject to the side conditions. -/\ndef AdmissibleMark (n : ℕ) (X : Finset ℝ) : Prop :=\n  (↑X ⊆ Set.Ioo (0 : ℝ) 1) ∧ X.card ≤ n\n\n/-- The value Liu Bang can guarantee.\n\n`V n` is the supremum over Liu Bang's admissible markings `A` of the infimum,\nover Xiang Yu's admissible markings `B` disjoint from `A`, of `L A B`. -/\nnoncomputable def V (n : ℕ) : ℝ :=\n  ⨆ A : {A : Finset ℝ // AdmissibleMark n A},\n    ⨅ B : {B : Finset ℝ // AdmissibleMark n B ∧ Disjoint A.1 B}, L A.1 B.1\n\n/-- The claimed answer value `V(n) = 2^n / (2^(n+1) - 1)`. -/\nnoncomputable def answer (n : ℕ) : ℝ := (2 : ℝ) ^ n / ((2 : ℝ) ^ (n + 1) - 1)\n\n/-! ## Correctness statements for the definitions\n\nThese pin down that the encoded definitions behave as intended. -/\n\n\n\n\n/-! ## Main Statements -/",
        statement="theorem L_mem_Icc (A B : Finset ℝ)\n    (hA : ↑A ⊆ Set.Ioo (0 : ℝ) 1) (hB : ↑B ⊆ Set.Ioo (0 : ℝ) 1) :\n    L A B ∈ Set.Icc (0 : ℝ) 1 := by",
        difficulty="imo",
        description="IMO 2026 P3, specification lemma. Liu Bang's share is a subset-sum of the piece lengths, hence lies in [0, 1].",
        tags=["imo", "imo2026", "q3", "combinatorics", "spec-lemma", "real", "spec"],
        source="benchmarks/imo2026/IMO2026/Q3/problem.lean (L_mem_Icc)",
    ),
    # IMO 2026 P3. In the Liu Bang / Xiang Yu stick-marking game with at most n marks each,
    # the length Liu Bang can guarantee is exactly 2^n / (2^(n+1) - 1).
    EvalProblem(
        name="imo2026_q3",
        statement_preamble="set_option backward.isDefEq.respectTransparency false\n\nopen scoped BigOperators\n\n\n/-- The multiset of piece lengths obtained by cutting `[0,1]` at the points of a\nfinite set `S ⊆ (0,1)`.  We sort `S` ascending, prepend `0` and append `1`, and\ntake consecutive differences.  The result is a list of `|S| + 1` positive reals\nsumming to `1` (when `S ⊆ (0,1)`). -/\nnoncomputable def pieceLengths (S : Finset ℝ) : List ℝ :=\n  let l : List ℝ := (0 : ℝ) :: (S.sort (· ≤ ·)) ++ [1]\n  List.zipWith (fun a b => b - a) l l.tail\n\n/-- The sum of the entries of a list `L` at the (0-indexed) even positions, after\nsorting `L` in non-increasing order.  These are the entries in the `1`st, `3`rd,\n`5`th, … positions of the sorted (decreasing) list, i.e. the pieces claimed by\nthe first mover under the greedy claiming rule. -/\nnoncomputable def firstPlayerShare (L : List ℝ) : ℝ :=\n  let sorted := L.mergeSort (· ≥ ·)\n  ((sorted.zipIdx.filter (fun p => p.2 % 2 = 0)).map (fun p => p.1)).sum\n\n/-- `L(A,B)`: Liu Bang's total length, given Liu Bang's marks `A` and Xiang Yu's\nmarks `B`. -/\nnoncomputable def L (A B : Finset ℝ) : ℝ :=\n  firstPlayerShare (pieceLengths (A ∪ B))\n\n/-- The set of admissible markings for a player: a finite subset of `(0,1)` of\nsize at most `n`.  We encode it as a `Finset ℝ` subject to the side conditions. -/\ndef AdmissibleMark (n : ℕ) (X : Finset ℝ) : Prop :=\n  (↑X ⊆ Set.Ioo (0 : ℝ) 1) ∧ X.card ≤ n\n\n/-- The value Liu Bang can guarantee.\n\n`V n` is the supremum over Liu Bang's admissible markings `A` of the infimum,\nover Xiang Yu's admissible markings `B` disjoint from `A`, of `L A B`. -/\nnoncomputable def V (n : ℕ) : ℝ :=\n  ⨆ A : {A : Finset ℝ // AdmissibleMark n A},\n    ⨅ B : {B : Finset ℝ // AdmissibleMark n B ∧ Disjoint A.1 B}, L A.1 B.1\n\n/-- The claimed answer value `V(n) = 2^n / (2^(n+1) - 1)`. -/\nnoncomputable def answer (n : ℕ) : ℝ := (2 : ℝ) ^ n / ((2 : ℝ) ^ (n + 1) - 1)\n\n/-! ## Correctness statements for the definitions\n\nThese pin down that the encoded definitions behave as intended. -/\n\n\n\n\n/-! ## Main Statements -/",
        statement="theorem V_eq (n : ℕ) (hn : 0 < n) : V n = (2 : ℝ) ^ n / ((2 : ℝ) ^ (n + 1) - 1) := by",
        difficulty="imo",
        description="IMO 2026 P3. In the Liu Bang / Xiang Yu stick-marking game with at most n marks each, the length Liu Bang can guarantee is exactly 2^n / (2^(n+1) - 1).",
        tags=["imo", "imo2026", "q3", "combinatorics", "game-theory", "real"],
        source="benchmarks/imo2026/IMO2026/Q3/problem.lean (V_eq)",
    ),
    # IMO 2026 P3, lower half. Liu Bang has a marking guaranteeing at least 2^n / (2^(n+1) -
    # 1) against every disjoint response.
    EvalProblem(
        name="imo2026_q3_lower_bound",
        statement_preamble="set_option backward.isDefEq.respectTransparency false\n\nopen scoped BigOperators\n\n\n/-- The multiset of piece lengths obtained by cutting `[0,1]` at the points of a\nfinite set `S ⊆ (0,1)`.  We sort `S` ascending, prepend `0` and append `1`, and\ntake consecutive differences.  The result is a list of `|S| + 1` positive reals\nsumming to `1` (when `S ⊆ (0,1)`). -/\nnoncomputable def pieceLengths (S : Finset ℝ) : List ℝ :=\n  let l : List ℝ := (0 : ℝ) :: (S.sort (· ≤ ·)) ++ [1]\n  List.zipWith (fun a b => b - a) l l.tail\n\n/-- The sum of the entries of a list `L` at the (0-indexed) even positions, after\nsorting `L` in non-increasing order.  These are the entries in the `1`st, `3`rd,\n`5`th, … positions of the sorted (decreasing) list, i.e. the pieces claimed by\nthe first mover under the greedy claiming rule. -/\nnoncomputable def firstPlayerShare (L : List ℝ) : ℝ :=\n  let sorted := L.mergeSort (· ≥ ·)\n  ((sorted.zipIdx.filter (fun p => p.2 % 2 = 0)).map (fun p => p.1)).sum\n\n/-- `L(A,B)`: Liu Bang's total length, given Liu Bang's marks `A` and Xiang Yu's\nmarks `B`. -/\nnoncomputable def L (A B : Finset ℝ) : ℝ :=\n  firstPlayerShare (pieceLengths (A ∪ B))\n\n/-- The set of admissible markings for a player: a finite subset of `(0,1)` of\nsize at most `n`.  We encode it as a `Finset ℝ` subject to the side conditions. -/\ndef AdmissibleMark (n : ℕ) (X : Finset ℝ) : Prop :=\n  (↑X ⊆ Set.Ioo (0 : ℝ) 1) ∧ X.card ≤ n\n\n/-- The value Liu Bang can guarantee.\n\n`V n` is the supremum over Liu Bang's admissible markings `A` of the infimum,\nover Xiang Yu's admissible markings `B` disjoint from `A`, of `L A B`. -/\nnoncomputable def V (n : ℕ) : ℝ :=\n  ⨆ A : {A : Finset ℝ // AdmissibleMark n A},\n    ⨅ B : {B : Finset ℝ // AdmissibleMark n B ∧ Disjoint A.1 B}, L A.1 B.1\n\n/-- The claimed answer value `V(n) = 2^n / (2^(n+1) - 1)`. -/\nnoncomputable def answer (n : ℕ) : ℝ := (2 : ℝ) ^ n / ((2 : ℝ) ^ (n + 1) - 1)\n\n/-! ## Correctness statements for the definitions\n\nThese pin down that the encoded definitions behave as intended. -/\n\n\n\n\n/-! ## Main Statements -/",
        statement="theorem lower_bound (n : ℕ) (hn : 0 < n) :\n    ∃ A : Finset ℝ, AdmissibleMark n A ∧\n      ∀ B : Finset ℝ, AdmissibleMark n B → Disjoint A B →\n        (2 : ℝ) ^ n / ((2 : ℝ) ^ (n + 1) - 1) ≤ L A B := by",
        difficulty="imo",
        description="IMO 2026 P3, lower half. Liu Bang has a marking guaranteeing at least 2^n / (2^(n+1) - 1) against every disjoint response.",
        tags=["imo", "imo2026", "q3", "combinatorics", "game-theory", "real"],
        source="benchmarks/imo2026/IMO2026/Q3/problem.lean (lower_bound)",
    ),
    # IMO 2026 P3, upper half. Against every Liu Bang marking, Xiang Yu has a disjoint
    # response holding him to at most 2^n / (2^(n+1) - 1).
    EvalProblem(
        name="imo2026_q3_upper_bound",
        statement_preamble="set_option backward.isDefEq.respectTransparency false\n\nopen scoped BigOperators\n\n\n/-- The multiset of piece lengths obtained by cutting `[0,1]` at the points of a\nfinite set `S ⊆ (0,1)`.  We sort `S` ascending, prepend `0` and append `1`, and\ntake consecutive differences.  The result is a list of `|S| + 1` positive reals\nsumming to `1` (when `S ⊆ (0,1)`). -/\nnoncomputable def pieceLengths (S : Finset ℝ) : List ℝ :=\n  let l : List ℝ := (0 : ℝ) :: (S.sort (· ≤ ·)) ++ [1]\n  List.zipWith (fun a b => b - a) l l.tail\n\n/-- The sum of the entries of a list `L` at the (0-indexed) even positions, after\nsorting `L` in non-increasing order.  These are the entries in the `1`st, `3`rd,\n`5`th, … positions of the sorted (decreasing) list, i.e. the pieces claimed by\nthe first mover under the greedy claiming rule. -/\nnoncomputable def firstPlayerShare (L : List ℝ) : ℝ :=\n  let sorted := L.mergeSort (· ≥ ·)\n  ((sorted.zipIdx.filter (fun p => p.2 % 2 = 0)).map (fun p => p.1)).sum\n\n/-- `L(A,B)`: Liu Bang's total length, given Liu Bang's marks `A` and Xiang Yu's\nmarks `B`. -/\nnoncomputable def L (A B : Finset ℝ) : ℝ :=\n  firstPlayerShare (pieceLengths (A ∪ B))\n\n/-- The set of admissible markings for a player: a finite subset of `(0,1)` of\nsize at most `n`.  We encode it as a `Finset ℝ` subject to the side conditions. -/\ndef AdmissibleMark (n : ℕ) (X : Finset ℝ) : Prop :=\n  (↑X ⊆ Set.Ioo (0 : ℝ) 1) ∧ X.card ≤ n\n\n/-- The value Liu Bang can guarantee.\n\n`V n` is the supremum over Liu Bang's admissible markings `A` of the infimum,\nover Xiang Yu's admissible markings `B` disjoint from `A`, of `L A B`. -/\nnoncomputable def V (n : ℕ) : ℝ :=\n  ⨆ A : {A : Finset ℝ // AdmissibleMark n A},\n    ⨅ B : {B : Finset ℝ // AdmissibleMark n B ∧ Disjoint A.1 B}, L A.1 B.1\n\n/-- The claimed answer value `V(n) = 2^n / (2^(n+1) - 1)`. -/\nnoncomputable def answer (n : ℕ) : ℝ := (2 : ℝ) ^ n / ((2 : ℝ) ^ (n + 1) - 1)\n\n/-! ## Correctness statements for the definitions\n\nThese pin down that the encoded definitions behave as intended. -/\n\n\n\n\n/-! ## Main Statements -/",
        statement="theorem upper_bound (n : ℕ) (hn : 0 < n) :\n    ∀ A : Finset ℝ, AdmissibleMark n A →\n      ∃ B : Finset ℝ, AdmissibleMark n B ∧ Disjoint A B ∧\n        L A B ≤ (2 : ℝ) ^ n / ((2 : ℝ) ^ (n + 1) - 1) := by",
        difficulty="imo",
        description="IMO 2026 P3, upper half. Against every Liu Bang marking, Xiang Yu has a disjoint response holding him to at most 2^n / (2^(n+1) - 1).",
        tags=["imo", "imo2026", "q3", "combinatorics", "game-theory", "real"],
        source="benchmarks/imo2026/IMO2026/Q3/problem.lean (upper_bound)",
    ),
    # IMO 2026 P4. In the triangle-cutting game (Mulan cuts along a cevian, Shan-Yu discards
    # one piece), Mulan can force a triangle with interior angle θ if and only if θ = 180 / n
    ...
    EvalProblem(
        ...
        statement="theorem main_theorem (θ : ℝ) (hθ0 : 0 < θ) (hθ180 : θ < 180) :\n    MulanCanGuarantee θ ↔ ∃ n : ℕ, 2 ≤ n ∧ θ = 180 / n := by",
        difficulty="imo",
        description="IMO 2026 P4. In the triangle-cutting game (Mulan cuts along a cevian, Shan-Yu discards one piece), Mulan can force a triangle with interior angle θ if and only if θ = 180 / n for some integer n ≥ 2.",
        tags=["imo", "imo2026", "q4", "combinatorics", "game-theory", "geometry"],
        source="benchmarks/imo2026/IMO2026/Q4/problem.lean (main_theorem)",
    ),
    # IMO 2026 P5. A function f on the positive reals satisfies sqrt((x² + f(y)²)/2) ≥ (f(x) +
    # y)/2 ≥ sqrt(x·f(y)) for all x, y if and only if f(x) = x + c for some c ≥ 0.
    EvalProblem(
        name="imo2026_q5",
        ...
        statement="theorem main_theorem (f : PositiveReal → PositiveReal) :\n    IsAdmissible f ↔\n      ∃ c : ℝ, 0 ≤ c ∧ ∀ x : PositiveReal, (f x : ℝ) = (x : ℝ) + c := by",
        difficulty="imo",
        description="IMO 2026 P5. A function f on the positive reals satisfies sqrt((x² + f(y)²)/2) ≥ (f(x) + y)/2 ≥ sqrt(x·f(y)) for all x, y if and only if f(x) = x + c for some c ≥ 0.",
        tags=["imo", "imo2026", "q5", "algebra", "functional-equation", "inequality", "real"],
        source="benchmarks/imo2026/IMO2026/Q5/problem.lean (main_theorem)",
    ),
    # IMO 2026 P6. For the gcd-greedy sequence (each term the least integer above the previous
    # sharing a factor with every earlier term), there are positive T, L with a(n+T) = a(n) +
    # L for all n.
    EvalProblem(
        name="imo2026_q6",
        ...
        statement="theorem main_theorem (a : ℕ → ℕ) (ha : IsValidSeq a) :\n    ∃ T L : ℕ, 0 < T ∧ 0 < L ∧ ∀ n, a (n + T) = a n + L := by",
        difficulty="imo",
        description="IMO 2026 P6. For the gcd-greedy sequence (each term the least integer above the previous sharing a factor with every earlier term), there are positive T, L with a(n+T) = a(n) + L for all n.",
        tags=["imo", "imo2026", "q6", "number-theory", "gcd", "periodicity"],
        source="benchmarks/imo2026/IMO2026/Q6/problem.lean (main_theorem)",
    ),
]
